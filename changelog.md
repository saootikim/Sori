---v1.2.0
새 기능
- 나를 위한 믹스: 자주 듣는 아티스트별로 매일 새로 만들어지는 데일리 믹스 (좋아하는 곡 + 비슷한 곡)
- 인기 차트: YouTube 공식 주간 차트 Top 100 (한국, 글로벌)과 인기 아티스트, 순위 변동 표시. 홈 "이번 주 인기곡"과 검색 "인기 차트" 타일
- 새 발매: 구독하거나 자주 듣는 아티스트의 새 앨범·싱글 모아보기, 새로 나오면 하루 한 번 알림 (새 발매 화면에서 끌 수 있음)

그 밖에
- 한국어 화면에서 앨범 발매 연도가 안 보이던 문제 수정

---v1.1.0
새 디자인
- 어두운 화면에 코랄 포인트 색, 굵은 제목
- 홈: 시간대 인사말, 맨 위 바로가기 타일과 "랜덤 재생", 홈 추천이 비어 있으면 Sori 추천 재생목록
- 검색: 검색어가 없으면 장르 타일 ("모두 둘러보기")
- 재생 화면과 미니 플레이어: 앨범 색 배경, 정사각 앨범아트, 얇은 진행 막대
- 재생목록과 앨범: 표지 색으로 물든 머리
- 보관함 "좋아요 표시됨" 새 표지
- 영상 썸네일의 검은 띠 제거

막혀 있던 기능 (무료 계정, 로그인 안 한 상태)
- 재생목록이 1곡만 나오던 문제: 전체 곡이 나옵니다
- 곡이 없던 앨범 페이지
- 노래 검색 결과가 비던 문제: 비디오 검색 결과로 보여줍니다
- 곡 라디오가 몇 곡에서 끝나거나 관련 없는 곡이 나오던 문제: 비슷한 곡으로 계속 이어집니다
- 아티스트 페이지: 인기곡, 전체 재생, 라디오, 셔플

그 밖에
- 한국어 가사 아래 로마자 줄 제거
- 번역이 빠져 있던 메뉴 한국어화
- 영상 제목의 "[MV]", "Official Video" 같은 꼬리 정리

설정에서 직접 바꾼 적 있는 항목은 그대로 유지됩니다.
